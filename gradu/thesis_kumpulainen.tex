...
%separate search term, search term usage tables?
...
topic categories is illustrated in Table \ref{table:topicdescription}.  

TODO: The emergent categories should be explained somewhere, either here, or in
the actual results chapter. For now, here are the category descriptions.
...
\begin{table}[H]
  ...
  \begin{tabular}{p{0.05\linewidth} p{0.30\linewidth} p{0.65\linewidth}}
    ...
    19 & Human-like qualities & Ideas with basis on human qualities like emotion and empathy.\\ \hline
...
information on the kind of research done, answering RQ1. Wieringa's scheme is presented in Table \ref{table:wieringa}.
...
In Figure \ref{fig:yearlybar} the amount of yearly published articles and their venues
are visualized in a bar plot. As the inspected time period on this thesis is
short, only five years, predicting temporal trends accurately is not possible.
However, it can be seen that every year there is steady publication pace of at
least 13 articles, with the average being 18.4 a year. In year 2018 25
different articles were published, which was the highest number during the
inspection period.

TODO: More text? Something about ICAGI?

Therefore to answer research question RQ2, It can be said that while there are AGI studies being published in different publication forums, vast majority of the research is released as yearly conference proceedings of International Conference on Artificial General Intelligence. Research community is active but small, as only average of 18,4 articles are published yearly.
...
Here is text telling about the topic categories, observations about them and
their relations to classes.
...
\begin{figure}[H]
  ...
  \begin{tikzpicture}
    ...
    \end{axis}
  \end{tikzpicture}
  \caption{Bubble plot showing article distribution between topics and article types}
  \label{topicbubble}
\end{figure}
