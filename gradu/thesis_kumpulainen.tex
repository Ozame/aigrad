...
papers were considered potential.
...
itself to general intelligence. In the end, 93 papers was included and would be
used as the mapping sample.

TODO: Table showing exclusion criteria, accepted percentages etc? Or in the
results chapter?
...
\pgfplotstableread[col sep=comma, header=true]{material/data/class_topic_bubble.dat}\forumDataTable
\def\forumCoordsFirst{}
\createSymbolicCoords{\forumCoordsFirst}{\forumDataTable}{Category}
\begin{figure}
  \begin{tikzpicture}
    \begin{axis}[
      only marks,
      ...
      \addplot[point meta=explicit] table[x={Class},y={Category},meta={Count},col sep=comma] {\forumDataTable};
    \end{axis}
  \end{tikzpicture}
  \caption{Bubble plot for article distribution between topics and article types}
  \label{kuvaFoorumit1}
\end{figure}
